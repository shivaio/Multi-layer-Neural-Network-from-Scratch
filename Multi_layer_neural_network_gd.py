...
class Neural_Network():
    ...
    def learn(self, training_inputs,training_outputs,learning_rate):
        ...
        error = self.cross_entropy_delta(estimated_output,training_outputs)
    ...
    def train(self,training_inputs,training_outputs,epochs, learning_rate ):
        ...
        categorical_training_outputs = self.to_categorical(training_outputs)


        # total_error = 0
        for epoch in range(epochs):
            total_error = 0
            for index_training_input in range(len(training_inputs)):
                # print(training_inputs[index_training_input].shape)
                # print(categorical_training_outputs[index_training_input][:].shape)
                training_input = training_inputs[index_training_input].reshape((1,training_inputs[index_training_input].shape[0]))
                training_output = categorical_training_outputs[index_training_input].reshape((1,categorical_training_outputs[index_training_input].shape[0]))
                # print(training_input.shape)
                # print(training_output.shape)
                final_weights,final_bias,total_error_each = self.learn(training_input,training_output,learning_rate)
